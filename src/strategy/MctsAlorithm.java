...
public class MctsAlgorithm {
    ...
    Node treePolicy(Node root) {
        while (nodeIsNonTerminal(root)) {
            if (nodeIsNotFullyExpanded(root)) {
                ...
                root.getGame().getActivePlayer().performMove(root.getMoveInNode());
                root.setUntriedMoves(new ArrayDeque<Move>(root.getGame().getActivePlayer().getAvailableMoves()));
            }
        }
        return root;
    }
    ...
    Node expand(Node root) {
        ...
        root.addChild(child);
        root.getGame().getActivePlayer().performMove(child.getMoveInNode());
        child.setUntriedMoves(new ArrayDeque<Move>(root.getGame().getActivePlayer().getAvailableMoves()));
        return child;
    }
