...
public class MctsAlgorithm {
    ...
    public Move run() {
        ...
        long end = start;
        Move bestFound = null;
        while (end - start <= timeForMctsMove * 100) {
    ...
    Node treePolicy(Node root) {
        while (nodeIsNonTerminal(root)) {
            if (nodeIsNotFullyExpended(root)) {
                return expand(root);
            } else {
    ...
    int defaultPolicy(Node root) {
        ...
        if (copy.getWinner().getName().equals(root.getGame().getFirstHero().getName())) {
            ...
            throw new IllegalStateException("Invalid winned. #1");
        }
    }
    ...
    void backup(Node nodeToBackup, int delta) {
        ...
        while (nodeToBackup != null) {
            if (delta == 1) {
                ...
                nodeToBackup.addSecondHeroWins();
            }
    ...
    private boolean nodeIsNotFullyExpended(Node root) {
